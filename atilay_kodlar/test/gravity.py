...
# Verileri okuyan ve global değişkene atan thread
def lidar_thread():
    global R 
    while True:
        R = atilay.lidar_on()
        time.sleep(0.01)  # Her 1 saniyede bir veri oku
...
if bmx160.begin():
    ...
    while True:
        ...
        #print(f"Manyetik Alan X: {data[0]:.2f} µT")
        #print(f"Manyetik Alan Y: {data[1]:.2f} µT")
        #print(f"Manyetik Alan Z: {data[2]:.2f} µT")
        #print(pusula(data[0],data[1]))
        #print(f"Jiroskop X: {data[3]:.2f} °/s")
        #print(f"Jiroskop Y: {data[4]:.2f} °/s")
        #print(f"Jiroskop Z: {data[5]:.2f} °/s")
        hareket(data[6],data[7],data[8])
        #print("gyz :",gyz)
        #print("gxz :",gxz)
        
        R2=np.cos(np.deg2rad(gxz))*int(R)
        R2=np.cos(np.deg2rad(gyz))*int(R2)
        #print("--------------")
        #print(R2)
        ...
        #print("yolz :",yolz)
        #print(R)
        fark=yolz-ilk_derece
        ilk_derece=yolz
        #R=lidarx.read_data_x()
        #print(fark)
        if giris:
            radyan_aci = np.deg2rad(yolz)
            print(yolz)
            y.append(np.sin(radyan_aci)*int(R))
            x.append(np.cos(radyan_aci)*int(R))
        ...
        if degisim:
            ...
            radyan_aci = np.deg2rad(yolz)
            #print(R)
            y.append(np.sin(radyan_aci)*int(R2))
            x.append(np.cos(radyan_aci)*int(R2))

        #print(f"İvmeölçer X: {data[6]:.2f} g")
        #print(f"İvmeölçer Y: {data[7]:.2f} g")
        #print(f"İvmeölçer Z: {data[8]:.2f} g")
...
# Dosya yolu
dosya_yolu = "/home/rasp/atilay/veriler.csv"

# CSV dosyasını açma ve yazma
df.to_csv(dosya_yolu, index=False) 

print(f"Veriler {dosya_yolu} adlı CSV dosyasına kaydedildi.")
